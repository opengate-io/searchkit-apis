...
function computeMinMax(items, field) {
    if (!items || items.length == 0)
        return { min: 0, max: 0 };
    return {
        min: lodash_4.minBy(items, field)[field],
        max: lodash_5.maxBy(items, field)[field]
    };
}
var TagCloud = (function (_super) {
    ...
    TagCloud.prototype.render = function () {
        ...
        var sortedItems = lodash_3.sortBy(items, function (it) { return translate(it.title || it.label || it.key).toLowerCase(); });
        var _b = computeMinMax(items, "doc_count"), min = _b.min, max = _b.max;
        return (React.createElement("div", { className: bemBlocks.container().mix(className).state({ disabled: disabled }) }, lodash_1.map(sortedItems, function (item) { return _this.renderItem(item, bemBlocks, min, max); })));
    };
    TagCloud.prototype.renderItem = function (item, bemBlocks, min, max) {
        ...
        return _1.renderComponent(itemComponent, {
            ...
            active: lodash_2.includes(selectedItems, item.key),
...
TagCloud.defaultProps = {
    ...
    minFontSize: 1,
    maxFontSize: 1.5,
    translate: lodash_6.identity,
    countFormatter: lodash_6.identity
};
TagCloud = __decorate([
    _1.PureRender
], TagCloud);
